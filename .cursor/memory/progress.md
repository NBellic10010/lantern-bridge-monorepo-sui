...
| # | 发现 | 处理 |
|---|------|------|
| 1 | Navi deposit PTB 应该用后端 SDK，不是链上 moveCall | 已在 Relayer 层实现 |
| 2 | vault.n_token 实际存的是 USDC Balance，不是 nUSDC | 已更新注释澄清 |
| 3 | YieldMode.YIELD 只在 EVM → Sui 方向有意义 | 已确认，Sui → EVM 无 Navi 可存 |
| 4 | buildSuiTransferTx 里有多余的 transferObjects | 用户已手动修复 |
| 5 | Sui → EVM transfer 里的 msgId 是人工生成的假 ID | 待后续处理（从事件解析） |

#### 已完成的代码修改

##### 1. admin.move — 管理员权限加固

- **文件**: `lantern-contracts/sources/admin.move`
- **改动**:
  - `Config` 结构体新增 `admin: address` 字段
  - `initialize` 初始化时设置 `admin: ctx.sender()`
  - `verify_admin` 实现真正的验证（cap 存在性 + sender 与 config.admin 比对）
  - 所有管理函数签名加 `ctx: &mut TxContext` 参数
  - 所有管理函数调用 `verify_admin(cap, config, ctx)`
  - 新增 `get_admin(config: &Config): address` getter

##### 2. vault.move — 手续费路由 + Config 集成

- **文件**: `lantern-contracts/sources/vault.move`
- **改动**:
  - 导入 `admin::{Config, get_fee_rate, get_treasury}`
  - `deposit` 函数签名加 `config: &Config` 参数
  - `deposit` 函数内实现 fee 路由：`fee = amount * fee_rate / 10000`，fee coin 转入 treasury
  - `withdraw` 函数签名加 `config: &Config` 参数
  - `withdraw` 函数内实现 fee 路由：`fee = assets * fee_rate / 10000`，fee 转入 treasury
  - 更新 `n_token` 注释：明确 vault 是纯记账层，USDC 实际由 Relayer 存入 Navi
  - 启用 `WithdrawEvent` emit（deposit 的 emit 保留 TODO 注释）

##### 3. relayer.service.ts — Navi Yield 集成

- **文件**: `lantern-backend/src/services/relayer.service.ts`
- **package.json**: 新增 `@naviprotocol/lending: ^1.0.0` 依赖
- **改动**:
  - `relayEvmNttToSui` 末尾：redeem 成功后从 Redis 查 `yieldMode`
  - 如果 `yieldMode === 'yield'`：从 `objectChanges` 找刚 mint 的 USDC Coin object ID，调用 `depositToNavi`
  - 新增 `getTransferReceiptByMessageId()`: 从 Redis 按 messageId 查 receipt（含 yieldMode）
  - 新增 `depositToNavi()`: 用 `@naviprotocol/lending` 的 `depositCoinPTB` 构建并执行存款 PTB
  - Navi 存款失败不阻断主流程（只 log error）

#### 架构澄清

```
EVM → Sui (YIELD 生效):
  executeEvmTransfer() → Wormhole Relayer → relayEvmNttToSui()
    → ntt_manager::redeem() [mint USDC]
    → depositToNavi() [depositCoinPTB → Navi vault → 获得 nUSDC]
    → nUSDC 归 Relayer 持有（当前实现）

Sui → EVM (YIELD 不适用):
  executeSuiTransfer() → Wormhole Relayer → relayNttMessage()
    → 在 Arbitrum mint USDC 完成转账
    → 无 Navi 存款（Arbitrum 没有 Navi）
```

#### 待完成

- [ ] `npm install @naviprotocol/lending`（手动执行）
- [ ] Navi SDK USDC coinType 确认（当前用硬编码值，需对照 Navi 主网实际地址）
- [ ] 集成测试（测试网）
